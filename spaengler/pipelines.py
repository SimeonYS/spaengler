...
class SpaenglerPipeline:

    # Database setup
    conn = sqlite3.connect('spaengler.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `spaengler`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM spaengler WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO spaengler (date, title, link, content)"
